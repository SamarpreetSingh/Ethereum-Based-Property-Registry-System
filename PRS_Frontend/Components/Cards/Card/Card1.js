...
const Card1 = ({ heading, subheading, img }) => {
  return (
    <div className="relative flex flex-col py-14 lg:flex lg:flex-row lg:items-center lg:py-6 ">
      <div className="order-last lg:order-first lg:w-2/4  lg:pl-28 lg:pr-10">
        <h4 className="-card-heading-color-1 mt-5 w-full text-center  font-inter text-[42px] font-[700]  leading-tight lg:text-left">
          {heading}
        </h4>
      ...
      <div className="order-1 flex items-center justify-center lg:order-2 lg:w-2/4">
        <img src={img} alt="card-image" className="card-image" />
      </div>
      <img
        src="/Assets/grad.png"
        alt="gradient"
        className="absolute -left-32 bottom-4 hidden md:block"
      />
    </div>
  )
}
